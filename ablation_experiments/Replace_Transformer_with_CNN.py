...
class Multiscale_Stage(nn.Module):
    def __init__(self, embed_dim, out_embed_dim, num_layers=1, num_heads=8, mlp_ratio=3,
        ...
        # )
        self.InvRes1 = ResBlock(in_features=embed_dim, out_features=embed_dim)
        self.InvRes2 = ResBlock(in_features=embed_dim, out_features=embed_dim)
    ...
    def forward(self, guide):
        ...
        #attn = self.mhca_blks(x, (H, W)) #(1,64,64,64)
        attn = self.InvRes1(guide)
        conv = self.InvRes2(guide) #(1,64,64,64)
        fuse = torch.cat([attn, conv], dim=1) #(1,128,64,64)
...
class MPViT(nn.Module):
    ...
    def __init__(
        ...
        self.prms = nn.ModuleList([
            PRM(in_chans=embed_dims[idx-1] if not idx == 0 else 64,
            embed_dim=embed_dims[idx] if not (idx + 1) == self.num_stages else embed_dims[idx],
            kernel_size=3,
            downsample_ratio=2)
            for idx in range(self.num_stages)
        ])
...
if __name__ == "__main__":
    ...
    with torch.no_grad():  # 在推理模式下，我们通常不需要计算梯度
        output = model(input)
    end_time = time.time()
