...
def print_model_parameters(model):
    total_params = 0  # 总参数量
    print(f"{'Layer Name':<30} {'Parameter Shape':<30} {'Parameter Count':<20}")
    print("=" * 80)

    for name, param in model.named_parameters():
        if param.requires_grad:  # 只统计可训练参数
            param_count = param.numel()  # 当前层的参数量
            total_params += param_count
            print(f"{name:<30} {str(param.shape):<30} {param_count:<20}")

    print("=" * 80)
    print(f"Total Trainable Parameters: {total_params}")
    print(f"Total Trainable Parameters (M): {total_params / 1e6:.2f} M")
