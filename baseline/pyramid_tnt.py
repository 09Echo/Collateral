...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 3, 'input_size': (3, 256, 256), 'pool_size': None,
...
default_cfgs = {
    'tnt_ti_patch16_192': _cfg(
        input_size=(1, 512, 512), mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
    ),
...
def ptnt_ti_patch16_192(pretrained=False, **kwargs):
    ...
    model = PyramidTNT(configs=configs, img_size=512, in_chans=1, num_classes=2, qkv_bias=False, **kwargs)
...
def ptnt_s_patch16_256(pretrained=False, **kwargs):
    ...
    model = PyramidTNT(configs=configs, img_size=512, in_chans=1, num_classes=3, qkv_bias=False, **kwargs)
...
if __name__ == '__main__':
    ...
    # model = ptnt_b_patch16_256()
    # out = model(x)
    # print(out.shape)
    import torch
    from thop import profile

    model = ptnt_s_patch16_256()
